...
| keyboardOptions    | Define as propriedades do [teclado virtual](../Keyboard/README.md) no momento da criação do componente `<Input />`. Tenha cuidado se tiver mais de um campo de entrada na mesma página, pois essas são propriedades globais do teclado (O que se aplicar para um `<Input />`, será aplicado para todos)            | `object`       | `{}`        |
