...
module.exports = {
  ...
  builder: (yargs) => {
    if (PKG == null) {
      console.error(chalk.red('No "package.json" found. Is this a mamba app directory?"'));
      process.exit(1);
    }
    ...
    if (typeof PKG.mamba === 'undefined') {
      console.error(
        chalk.red(`No "mamba" property found at "${PKG.rootDir}" Is this an mamba app directory?"`),
      );
      process.exit(1);
    }
    ...
    process.env.PATH +=
      (process.platform === 'win32' ? ';' : ':') + fromCwd('node_modules', '.bin');
